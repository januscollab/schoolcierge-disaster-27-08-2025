...
console.log(`
${color.title('🚀 SchoolCierge Task Management System')}
...
${color.section('Task Status Values:')}
  • not-started ${color.desc('(default)')}
  • in-progress
  • blocked
  • completed

${color.section('Priority Levels:')}
  • P0 ${color.desc('(Critical - Must have)')}
  • P1 ${color.desc('(High - Should have)')}
  • P2 ${color.desc('(Medium - Nice to have)')}
  • P3 ${color.desc('(Low - Future consideration)')}

${color.section('Categories:')}
  • infrastructure  • authentication  • backend
  • integration     • mobile          • devops
  • clara          • timer           • adapt
  • admin
...
if (fs.existsSync(tasksPath)) {
  ...
  console.log(`  📊 Completion: ${tasks.length > 0 ? Math.round(completed / tasks.length * 100) : 0}%`);
}

console.log(`
${color.section('Need more help?')}
  ${color.desc('• PRD location:')} .project/product/PRD-DEV.md
  ${color.desc('• TRD location:')} .project/product/TRD-DEV.md
  ${color.desc('• Task data:')}    .project/tasks/backlog.json
  ${color.desc('• Scripts:')}       .project/scripts/
`);